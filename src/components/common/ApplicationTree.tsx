...
export default function ApplicationTree() {
  ...
  return (
    <View className="mx-3 mt-2 rounded-b-md border border-t-0 px-3 py-3" style={{ borderColor: palette.border, backgroundColor: palette.surfaceRaised }}>
      <Text className="mb-1 font-semibold uppercase tracking-wide" style={[typography.styles.caption, { color: palette.textMuted }]}>Application Tree</Text>
      <Text className="mb-3 text-xs font-semibold uppercase tracking-wide" style={{ color: '#19A85A' }}>
        Pending
      </Text>
      ...
      <View className="gap-y-2">
        {treeRows.map((row, index) => {
          return (
            <View key={`${row.label}-${row.value}`} className="flex-row items-center rounded-lg border px-3 py-2.5" style={{ borderColor: palette.border, backgroundColor: palette.surface }}>
              <Text className="flex-1 pr-3 text-sm font-medium" style={{ color: palette.textPrimary }} numberOfLines={1}>
                {row.label}
              </Text>
              <View
                className="min-w-20 rounded-md px-3 py-1.5"
                style={{ backgroundColor: index === 0 ? '#47566D' : '#3C4A5F' }}
              >
                <Text className="text-right text-xs font-semibold" style={{ color: '#D9E8FF' }} numberOfLines={1}>
                  {row.value}
                </Text>
              </View>
            </View>
        ...
        <View className="rounded-xl border px-3 py-2.5" style={{ borderColor: '#1198F0', backgroundColor: '#0E4A86' }}>
          <View className="flex-row items-center justify-between gap-x-3">
            ...
            <View className="rounded-md bg-[#2B6FBB] px-3 py-1">
              <Text className="text-xs font-semibold" style={{ color: '#FFFFFF' }} numberOfLines={1}>
                Dulanjali. Ravindi
              </Text>
            </View>
          </View>
